...
export function MessageList({ messages, isGroup, onReply }: Props) {
  ...
  useEffect(() => {
    ...
    if (nearBottom || grew) bottomRef.current?.scrollIntoView({ behavior: grew ? 'smooth' : 'auto' });
  }, [messages.length]);
  ...
  return (
    <div ref={scrollRef} className="tg-scroll chat-pattern flex-1 overflow-y-auto py-3">
      {messages.map((m, i) => {
        ...
        const newDay = !prev || new Date(prev.date * 1000).toDateString() !== new Date(m.date * 1000).toDateString();
        const sameNext =
